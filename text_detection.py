...
def text_detection(image, east, min_confidence, width, height):
    ...
    for i, function in enumerate(functions):
        ...
        # np_array = drawPolygons(drawOn, drawpolys, ratioWidth, ratioHeight, (0, 255, 0), 2)
        # print(">>>>len(np_array):" + str(len(np_array)))
        # for i in range(len(np_array)-1):
        #     cv2.imshow("np_array[i]" , np_array[i])
            # np_array[i].save("numpy_altered_sample2_" + str(i) +".png")
        
        # cv2.imshow("title", imgCrop)
        # cv2.imshow(title,drawOn)
        cv2.moveWindow(title, 150+i*300, 150)
        cv2.waitKey(0)
    cv2.waitKey(0)
